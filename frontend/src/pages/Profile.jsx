...
export default function Profile() {
  ...
  return (
    ...
    >
      <header className="flex flex-col md:flex-row items-center gap-8 bg-white/5 p-10 rounded-[40px] border border-white/5 backdrop-blur-3xl relative overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

        <div className="w-32 h-32 rounded-[32px] bg-gradient-to-br from-slate-700 to-slate-900 border-2 border-primary/20 flex items-center justify-center text-4xl font-black text-primary italic shadow-2xl relative z-10">
          SC
        </div>

        <div className="text-center md:text-left relative z-10">
          <h1 className="text-4xl font-black text-white mb-2">Cientista de Dados</h1>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s, i) => (
          <motion.div
            ...
            className="bg-white/5 border border-white/5 p-6 rounded-[32px] hover:bg-white/10 transition-colors"
          >
            <span className="material-symbols-rounded text-primary mb-4 block text-3xl">{s.icon}</span>
            <p className="text-2xl font-black text-white">{s.value}</p>
            <p className="text-[10px] uppercase tracking-widest font-bold text-slate-500">{s.label}</p>
          </motion.div>
        ))}
      </section>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {certifications.map((cert, i) => (
              <div key={i} className="flex items-center justify-between p-6 bg-white/5 border border-white/5 rounded-3xl group hover:border-primary/20 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">Expira em: {cert.expiry}</p>
                  </div>
                </div>
                <button onClick={() => setPdfModal(cert)} className="text-[10px] font-black text-primary uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity hover:underline">Ver PDF</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white/5 border border-white/5 rounded-[40px] p-8 space-y-6">
            {teamMembers.map((member, i) => (
              <div key={i} className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-slate-800 border border-white/5 flex items-center justify-center text-xs font-black text-primary">
                  {member.initial}
                </div>
                <div>
            ...
            <button onClick={() => setShowInvite(true)} className="w-full py-4 rounded-2xl border border-white/10 text-[11px] font-bold text-slate-400 hover:bg-white/5 transition-all">Convidar Colaborador</button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showInvite && (
          ...
          >
            ...
            >
              <h2 className="text-xl font-black text-white mb-6">Convidar Colaborador</h2>
              {inviteSent ? (
                ...
                <form onSubmit={handleInvite}>
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setShowInvite(false)} className="flex-1 py-3 rounded-2xl border border-white/10 text-xs font-bold text-slate-400 hover:bg-white/5 transition-all">Cancelar</button>
                    <button type="submit" className="flex-1 py-3 rounded-2xl bg-primary text-black text-xs font-black uppercase tracking-widest hover:brightness-110 transition-all">Enviar</button>
                  </div>
                </form>
              )}
            </motion.div>
        ...
        {pdfModal && (
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
              className="glass-card w-full max-w-md rounded-[2rem] p-8 border-white/10 text-center"
            >
